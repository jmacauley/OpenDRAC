...
<%String pageRef = "drac.menu.service.logs";%>
...
<table width="98%" cellspacing="5" cellpadding="0" border="0" align="center">
    ...
    <tr>
        <td align="center">
        <logic:empty name="serLogList" scope="request">
            <bean:message key="drac.auditlogs.empty"/>
        </logic:empty>
        <logic:notEmpty name="serLogList" scope="request">
        <display:table id="log" name="serLogList" cellspacing="1" cellpadding="5" class="forumline" pagesize="20" sort="list" requestURI="">
                <c:set var="tdclass" value="row1c" />
                <c:if test="${log_rowNum % 2 == 0}">
                    <c:set var="tdclass" value="row2c" />
                </c:if>
            <display:column property="time" sortable="true" sortProperty="occurTime" title='<c:out value='${sessionScope["drac.auditlogs.list.time"]}'/>c:out value='${sessionScope["TZString"]}'/>
            ...
            <display:column property="resource" sortable="true" href="/service/serviceDetails.do" paramId="sid" paramProperty="resource" title='<c:out value='${sessionScope["drac.auditlogs.list.resource"]}'/>' class="${tdclass}"> </display:column>
