		...
		try {
			...
			if (result != null) {
				...
				while (resultIr.hasNext()) {
					...
					String ringName = aResult.get("LABEL");
					String ringMap = aResult.get("RINGMAP");
					String protectionScheme = aResult.get("PS");

					// process the apsId and the ringMap is expected to have the
					// following format 1:A/2:B/3:C/1:A

					String apsIdStr = "N/A";
					if (ringMap != null) {
						log.debug("got ringMap info for " + ringName + " in " + ringMap);
						apsIdStr = ringMap.split("/")[0].split(":")[0];
					}
					ne.setApsId(Integer.valueOf(apsIdStr));

					String workingAid = aResult.get("WORKINGAID");
					String protectionAid = aResult.get("PROTECTIONAID");
					// If it is the 4F_BLSR then then the aids need further work since
					// they
					// have the following format: OC192-1-501-0-3-1&OC192-1-501-0-4-1
					if (protectionScheme.toUpperCase().indexOf("4FR") >= 0) {
						String[] temp = workingAid.split("&");

						DbUtility.INSTANCE.updateOrAddFacilityAttr(ne, temp[0],
						    AbstractFacilityXml.APSID_ATTR, apsIdStr);
						DbUtility.INSTANCE.updateOrAddFacilityAttr(ne, temp[0],
						    AbstractFacilityXml.PROTECTIONSCHEME_ATTR, protectionScheme);
						DbUtility.INSTANCE.updateOrAddFacilityAttr(ne, temp[1],
						    AbstractFacilityXml.APSID_ATTR, apsIdStr);
						DbUtility.INSTANCE.updateOrAddFacilityAttr(ne, temp[1],
						    AbstractFacilityXml.PROTECTIONSCHEME_ATTR, protectionScheme);

						temp = protectionAid.split("&");
						DbUtility.INSTANCE.updateOrAddFacilityAttr(ne, temp[0],
						    AbstractFacilityXml.APSID_ATTR, apsIdStr);
						DbUtility.INSTANCE.updateOrAddFacilityAttr(ne, temp[0],
						    AbstractFacilityXml.PROTECTIONSCHEME_ATTR, protectionScheme);
					...
					else if (protectionScheme.toUpperCase().indexOf("2FR") >= 0) {
					...
					else if (protectionScheme.toUpperCase().indexOf("1PLUS1") >= 0) {
						DbUtility.INSTANCE.updateOrAddFacilityAttr(ne, workingAid,
						    AbstractFacilityXml.PROTECTIONSCHEME_ATTR, protectionScheme);
						DbUtility.INSTANCE.updateOrAddFacilityAttr(ne, protectionAid,
						    AbstractFacilityXml.PROTECTIONSCHEME_ATTR, protectionScheme);
					}
					DbUtility.INSTANCE.updateFacility(ne, data);
					log.debug("protection scheme= " + protectionScheme + "apsId= "
					    + apsIdStr);
				}
				return true;
			}
			...
			log.error("Fail to obtain the ring information", e);
		}
		// Not really want to stop the initialization process.
		return true;
	}
}