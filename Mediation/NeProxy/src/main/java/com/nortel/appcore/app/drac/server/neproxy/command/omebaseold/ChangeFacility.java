...
public final class ChangeFacility extends AbstractCommandlet {
	...
	public boolean start() {
		...
		while (ir.hasNext()) {
			...
			// Determine the owner of the attribute(s) (i.e it can be controlled
		...
		try {
			log.debug("Modify the Facility attribute in DB");
			...
			//
			// When a L2 (ETH/WAN) facility is 'configured' into DRAC, the reqt is to
			// silence alarms that would otherwise be raised while the facility
			// remains
			// unconnected. This step is accomplished by provisioning a unidirectional
			// loopback connection across the facility, named with a predetermined
			// label (used elsewhere for identifying the loopback as originating from
			// DRAC).
			//
			...
			// operation (only the logs will reflect the failure). This scenario could
			// occur, for example, if someone edits the facility while a real service
			// is still inProgress.
			//
			...
			if (signalType != null) {
				if (aid.indexOf(FacilityConstants.WAN) >= 0
				    || aid.indexOf(FacilityConstants.ETH) >= 0) {
					/*
					 * Put this in a new thread to ensure that the edit facility operation
					 * response time is not affected, particularly if the assoc is down
					 * (the ENT/DLT-CRS commands can be slow to return/timeout).
					 */
              	...
              	public void run() {
              		 ...
              		 * facility/port. e.g. ETH-1-1-1 The existing methods for adding
              		 * and deleting loopbacks deal with payload/channel aids. e.g.
              		 * ETH-1-1-1-1 We can safely append a payload value of '1' to this
              		 * aid (since this is a Layer 2 aid). Also, the aid type qualifier
              		 * at the front end is not important since it is stripped off by
              		 * the add/removeLoopback methods. I will just prepend 'na'.
              		 */
              		String modifyAid = "na"
              		...
              		if (FacilityConstants.SIGNAL_TYPE.UNI.toString().equals(
              		    signalType)) {
              			RemoveConnection.addLoopBack(ne, modifyAid);
              		}
              		else {
              			CreateConnection.removeLoopBack(ne, modifyAid);
              		}
              	}
