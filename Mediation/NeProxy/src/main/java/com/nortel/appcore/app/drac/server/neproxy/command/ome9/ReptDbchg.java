...
public final class ReptDbchg extends AbstractCommandlet {
	...
	private Map<String, String> getAndUpdateFacility(AbstractNe ne,
        ...
        if (log.isDebugEnabled()) {
            log.debug("getAndUpdateFacility: " + mod + ", aid=" + aid + ", oper=" + oper.asString());
        }
		...
		if (result != null) {
            log.debug("getAndUpdateFacility: processing result");
			...
			if (updateFacilityAttribute(ne, aResult, aid, oper)) {
                ...
                log.error("getAndUpdateFacility: updateFacilityAttribute() db update failed for ne=" + ne.getNeName() + ", aid=" + aid + ", oper=" + oper);
            }
		}
		else {
			log.error("Got empty result from querying new state of " + aid
			    + " from NE " + ne.getNeName());
		}
	...
	private boolean handleStateChangeEvent(AbstractNe ne,
        ...
        if (command.indexOf("STCHG-EQPT") >= 0 ||
            ...
            if (log.isDebugEnabled()) {
                log.debug("handleStateChangeEvent: RTRV_" + aidMap[0] + ", fromAid=" + fromAid + ", oper=" + oper.asString());
            }
	...
	private boolean updateFacilityAttribute(AbstractNe ne,
        ...
        /*
         * If this is a WAN facility then we will have special handling code
         * once we distinguish if we have an EPL or L2SS port.
         */
		if (Facility.isWan(aid)) {
			/*
             * If the WAN is a standalone, it is / will be explicitly stored
             * in the db. If it's part of an EthWan EPL port, it'll be the ETH
             * in our db. Therefore, do the lookup based on shelf/slot/port.
             *
             * Note: For Eth/Wan EPL-like facility pairs, we rely on the ETH
             * being processed ahead of the Wan.
             */
			boolean wanIntoEthMapping = false;
			Element changedElement = null;
			String[] aidArr = aid.split("-");

            /* Query the facility database using only shelf-slot-port. */
            changedFac = DbUtility.INSTANCE.retrieveAFacility(ne, aidArr[1], aidArr[2], aidArr[3]);

            /* Did we find a matching facility entry? */
			if (changedFac != null) {
                ...
                /* If we got an ETH facility back then we know it is not an
                 * L2SS port.
                 */
                ...
                /* We found no matching facility entry - what does this mean? */

                /* TODO: Figfure out the handling code needed here */
            }

            /* Now we process based on if an ETH/WAN or L2SS WAN. */
			if (wanIntoEthMapping) {
				/* This WAN facility belongs to an Eth/Wan EPL port.
                 *
				...
				/* DRAC FLAG: to indicate this ETH is actually a WAN-backed
                 * ETH (see GetAllEthWanFacility).
                 */
				newFacElem.setAttribute(FacilityConstants.IS_EPL, "true");
				currentNeAttrValue.put(FacilityConstants.IS_EPL, "true");
			}
			else {
				/* Standalone WAN - WAN as Layer2 selectable UNI. */
				layer = 2;
				layerStr = AbstractFacilityXml.LAYER2_ELEMENT;
				newFacElem = new Element(layerStr);
			...
			/* Nov.1, 2010
             *
             * At the time of writing, the OME R7 load was not emitting a
             * dbchange report for FE (ETH100) in such a way that the primary
             * state could be parsed.
             *
             * The ETH100 would only appear on toggling association (thereby
             * learning of the ETH100 via retrievals).
             *
            ...
            /*
             * If this facility is not a ETH or WAN it must me layer 1.
             * TODO: This will need to distinguish between layer 0 and layer 1
             * if we ever decide to model the layer 0 resources on OME.
             */
			layer = 1;
		...
		while (keyIr.hasNext()) {
			...
			if (aValue != null) {
                if (xmlKey.equals(AbstractFacilityXml.SPEED_ATTR)) {
                    /* Other Ethernet facilities return rates in Mb/s, however,
                     * for ETH10G it is supplied as 10G. Normalize to Mb/s ...
                     */
                    if (aid.indexOf("ETH10G") >= 0) {
                        aValue = "10000";
                    }
                    else if (aid.indexOf("ETH100G") >= 0) {
                        aValue = "100000";
                    }
                }
        ...
        /* We only did a retrieval if this was an WAS facility, so now we
         * need to retrieve for other types.
         */
		if (changedFac == null) {
        ...
        /* We are updating an existing facility. */
		else {
            /* We meed to deterime which attributes have changed for the update. */
			...
			if (updateList != null || oper != CHANGE.MODIFY) {
				/* If there are no changes and the operation is ADD or DELETE,
                 * then we still want to update the Valid field accordingly.
                 */
				if (updateList == null) {
				 ...
				 * an Eth/Wan pair enrol; when the WAN enrols, its attributes
                 * of interest will be added to its associated Eth.
                 */
                ...
                /* We have a modify operations but no change in attributes we
                 * are tracking.
                 */
                log.debug("updateFacilityAttribute: modify operation but no facility attributes have changed for ne=" + ne.getNeName() + ", aid=" + aid);
				return false;
			}
		}
