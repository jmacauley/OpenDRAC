 ...
 * @author GGL2
 */
public final class GetETH10GFacility extends AbstractCommandlet {
	public GetETH10GFacility(Map<String, Object> param) {
		super(param);
	}
		...
		try {
			...
			if (result != null) {
				...
				for (Map<String, String> aResult : result) {
					...
					// WARNING:
					// The 10G Ethernet facility returns with an attribute list
					// significantly different than
					// than of the regular GE. Until proven otherwise, assume the the
					// attribute keys that we
					// require in DRAC are common between the two (and that we don't
					// require the extended
					// attributes of the 10G ETH).
					...
					for (Map.Entry<String, String> e : OmeFacilityXML.ethNeToXmlMapping
						...
						if (aValue != null) {
							...
							if (e.getValue().equals(AbstractFacilityXml.SPEED_ATTR)) {
