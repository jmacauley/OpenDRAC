...
public class PostDeleteConnections extends AbstractCommandlet {
  private final Logger log = LoggerFactory.getLogger(getClass());
  
	private String targetNeId = null;
	private static final String EVENT_CODE = "POST-DLT-CONNECTIONS";

	public PostDeleteConnections(Map<String, Object> param) {
		super(param);

		// The AbstractCommandlet class carries a result class called Candidate. It
		// will
		// instantiate a new one if one is not provided via the input parameters
		// map.
		// L2SS activation relies on passing info back to the caller via the results
		// class.
		// In order to pass results up from the bottom, we need to
		// ensure that only one 'Candidate' class is used by the related commandlets
		// by putting
		// this Candidate into the parameters map (so that a new one will not get
		// created when
		// the specific commandlet is created).
	...
	public boolean start() {
		...
		try {
			...
			log.debug("PostDeleteConnections invoking commandlet " + className
			    + " for " + EVENT_CODE);
			...
			log.debug("PostDeleteConnections: commandlet returned false! "
			    + EVENT_CODE);
			getCandidate().setErrorCode("ERR_SIOE");
			return false;
		}
		catch (Exception e) {
			log.error("PostDeleteConnections: Failed to invoke handler", e);
		}
