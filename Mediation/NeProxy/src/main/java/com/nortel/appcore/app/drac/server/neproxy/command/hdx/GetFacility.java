...
public final class GetFacility extends AbstractCommandlet {
	...
	public boolean start() {
		try {
			...
			log.debug("Getting the facility from NE " + ne.getNeName());
			...
			if (result != null) {
				...
				if (currentData.length() > 0) {
					...
					facilityXML = new HdxFacilityXML(aDoc.getRootElement());
					// in case there are new attributes added to this releaswe
					facilityXML.populateStaticAttributes();
				}
				else {
					facilityXML = new HdxFacilityXML(ne);
				}
				...
				while (resultIr.hasNext()) {
					...
					attributeList.put(AbstractFacilityXml.PRIMARYSTATE_ATTR,
					    aResult.get("PST").toString());
			...
			log.error("Failed to get facility", e);
		}
		return false;
	}
}
